...
public final class GoogleReader implements
	...
	public void markItemAsUnStarred(String itemUrl,String feedId) throws GoogleReaderException{
     ...
     public List<ItemDescriptor> getFeedItems(String feedUrl) throws GoogleReaderException{
    	 List<ItemDescriptor> descriptions = GoogleReaderUtil.getItemDescriptorsFromItemsXml(api.getFeedItems(feedUrl, 10));
    	 Iterator<ItemDescriptor> it = descriptions.iterator();
    	 while(it.hasNext()){
    		 ItemDescriptor desc = it.next();
    		 desc.setDescription(GoogleReaderUtil.removeJavaEyeAds(desc.getDescription()));
    	 }
    	 
    	 return GoogleReaderUtil.getItemDescriptorsFromItemsXml(api.getFeedItems(feedUrl, 10));
